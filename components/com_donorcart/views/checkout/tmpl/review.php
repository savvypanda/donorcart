...
<div id="donorcart_checkout_container">
	<?php if(!$this->user->id && $this->params->get('require_email_for_guest_checkout',false)): ?>
		...
		<div><?=$this->order->email?></div>
	<?php endif; ?>
	<?php if($recurring_flag==1) { ?>
		<h3><?=JText::_('COM_DONORCART_CHECKOUT_HEADING_RECURRING')?></h3>
		<div><?=JText::_($this->order->recurring?'COM_DONORCART_CHECKOUT_RECURRING_YES':'COM_DONORCART_CHECKOUT_RECURRING_NO')?></div>
	<?php } ?>
	...
	<div id="donorcart_review">
		...
		<div><?=($this->order->special_instr?$this->order->special_instr:JText::_('COM_DONORCART_CHECKOUT_NO_SPECIAL_INSTR'))?></div>
